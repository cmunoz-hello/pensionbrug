...
All pension funds must comply by January 2028. The key change is that your Pillar 2 pension moves from a **defined benefit** (guaranteed fixed payout) to a **defined contribution** (personal investment pot).
...
**Defined Contribution (new system):** Your contributions are invested in a personal pot. The final amount depends on how those investments perform. You carry the investment risk — but you also benefit if markets do well."""
    ...
    if any(word in user_input for word in ["aow", "state pension", "pillar 1", "first pillar"]):
        return """The AOW is the Dutch state pension — Pillar 1. It is automatically accrued by anyone living or working in the Netherlands.

You build up 2% per year of Dutch residence, meaning 50 years gives a full benefit. The full AOW is approximately €19,674 per year for a single person (2025 figures).
...
- Freelancers and self-employed workers (no Pillar 2)
...
For Pillar 2: pension rights accrued with Dutch employers stay in Dutch funds. Transfer (waardeoverdracht) to a new fund is possible but must be requested manually within a time limit.
